...
\paragraph{}This chapter presents Sprint 3, which extended the generator from a standalone synthetic tool
...
while the connected generator receives grabbed data, prepares the artifacts to generate, validates
them, writes the \texttt{.DAT} files, and delegates SQL script assembly to the same
\texttt{SqlScriptGeneratorService} used by standalone mode.
...
\hline
Generation & \texttt{/connected/generation} & Generate the final \texttt{.DAT} files, assemble the SQL script, and download \texttt{load\_test\_data.sql}. \\
\hline
\end{tabular}
\end{table}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|P{0.28\linewidth}|Q{0.18\linewidth}|P{0.44\linewidth}|}
\hline
...
\paragraph{}The generation endpoint calls the same connected preparation logic, validates each non-empty
\texttt{.DAT} dataset through the \texttt{ValidatorService}, writes the files to the configured output
directory, creates the load summary, and finally delegates to the \texttt{SqlScriptGeneratorService}
to build \texttt{load\_test\_data.sql}. The generated script is returned as a file download from the
browser.
...
\hline
Output generation & Generated the final SQL script from connected artifacts. & Non-empty datasets are validated, written as \texttt{.DAT} files, and assembled into \texttt{load\_test\_data.sql}. \\
\hline
\end{tabular}
\end{table}
