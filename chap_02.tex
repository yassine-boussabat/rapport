...
product. In its standard form, Scrum defines three core roles, three
artifacts, and four ceremonies that punctuate each sprint cycle, as
illustrated in Figure~\ref{fig:scrum}.
...
\paragraph{}Sprint 1 and Sprint 5 were planned as two-week sprints, while
Sprint 2, Sprint 3, and Sprint 4 were planned as four-week sprints. This
distribution gave more time to the implementation-heavy phases of the project:
the core generation engine, connected mode, and LLM enrichment.
...
\begin{itemize}
    ...
    closer to the target use case because it generates SQL \texttt{INSERT}
    ...
    pre-BCP buffer mechanism (the intermediate loading layer where records
    are staged before Longview stored procedures apply them), and no way to
    produce the specific Unicode casting and type conventions the platform
    requires.
\end{itemize}
...
QA Engineer
  & \begin{itemize}[noitemsep, topsep=2pt, leftmargin=*]
      ...
      \item Provide SQL Server credentials and test connectivity in
            connected mode
      \item Browse and select existing account groups, accounts, and
            securities to anchor generated records in connected mode
...
\begin{itemize}
    \item \textbf{Performance:} Standalone generation should complete
    quickly for a typical QA dataset. LLM-assisted generation may take
    longer due to network round-trips, but the experience must remain
...
\centering
\makebox[\textwidth][c]{%
\includegraphics[width=1.5\textwidth,height=0.92\textheight,keepaspectratio]{img/global-package-diagram.pdf}}
...
\begin{itemize}
    ...
    \item The \textbf{output directory} stores the final generated SQL loading
    script.
\end{itemize}
...
\hline
11 & Connected Mode
  & As a QA engineer, I want to test, open, and close a read-only
    connection to a live Longview instance, grab account groups and
    accounts, fetch securities and reference data, and preview
    generation so that I can anchor new data to existing entities
    before export. \\
\hline
12 & LLM Enrichment
  & As a QA engineer, I want to optionally enrich account groups,
    accounts, models, and securities with AI-assisted labels so that
    the generated dataset is more realistic and readable. \\
\hline
13 & Configuration Library
  & As a QA engineer, I want to save, reload, rename, delete, and
    filter compatible generation configurations so that I can reuse
    scenarios without re-entering parameters each time. \\
\hline
14 & Validation and Error Handling
  & As a QA engineer, I want the interface to validate inputs, show
    warnings and clear errors, and expose generation logs so that I
    can correct issues and monitor execution. \\
\hline
...
% ── Tailwind CSS ───────────────────────────────
\paragraph{}\textbf{Tailwind CSS + Radix UI + lucide-react}\\
...
stylesheets. It was complemented by Radix UI primitives for accessible form
controls, dialogs, progress indicators, and selection components, while
\texttt{lucide-react} provided the icon set used throughout the interface.
Custom React components were then built on top of these libraries to keep the
application consistent with its own design system.
\begin{figure}[H]
\centering
...
first-class and helps keep backend contracts and frontend integration
strongly typed and easier to maintain.
\begin{figure}[H]
\centering
...
% ── StarUML ────────────────────────────────────
\paragraph{}\textbf{StarUML}\\
StarUML is a UML modeling tool used to design and document software
architecture through standard diagrams. In this project, it was used to
produce the use case, package, physical architecture, class, and sequence
diagrams included throughout the report, helping formalize both the analysis
and the system design.
\begin{figure}[H]
\centering
\includegraphics[width=0.12\textwidth]{img/logo-staruml.png}
\caption{StarUML logo}
\label{fig:logo-staruml}
\end{figure}
